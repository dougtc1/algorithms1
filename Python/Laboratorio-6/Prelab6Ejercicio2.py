...
def sumfact(N:int)->(int):
	# Precondicion: N>=0
	# Postcondicion: sumfact(N)==sum(prod(j for j in range (1,x))) for x in range (0,k)

	# var	suma : int // Variable que almacena el valor de la suma de los factoriales
	# var	fact : int // Variable que almacena el valor del factorial
	# var	cota : int // Variable que establece la cantidad de veces a iterar
	# var	k : int //	Variable de iteracion
...
try:
	assert (sumfact(N)==sum (prod (j for j in range (1,x))) for x in range (0,k))
except:
	print ("La postcondicion no se cumple")
	quit()

# Salida
print ("La suma de los factoriales del numero",N,"es",sumfact(N))
