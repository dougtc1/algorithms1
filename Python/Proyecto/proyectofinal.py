...
# len(TableroCuadros)==(filas-1)*(columnas-1)"""
 ...
 3 para guardar o cualquier otra tecla para salir: "))
        ...
        turno = movida(TableroHorizontal,TableroVertical,TableroCuadros,filas,columnas,turno,1)

        print("Post-BIND JUGADOR 1")
...
# Definicion de funcion que marca una linea en el canvas y respectiva matriz
#------------------------------------------------------------------------------#
...
# Postcondicion:
    ...
    for i in range(filas):
        for j in range(columnas):
            ...
            startx = 48+(50*i+((150-(filas*10))*2.5))+r # >= for any i in range (filas-1)
            starty = 48+(50*j+((100-(columnas*10))*2.5))+r  # == for any j in range (columnas-1)

            # Verificacion si el jugador marco una linea horizontal
            if ((clickx-startx<=50) and starty-r<=clicky<=starty+r):
                
                TableroHorizontal[j][i] = "1"
                ...
                HacerCuadros(TableroHorizontal,TableroVertical,\
                                        TableroCuadros,filas,columnas,jugador,\
                                        turno)
                return canvas.create_line(startx,starty,startx+50,starty,width=3,fill=color)

                              
            # Verificacion si el jugador marco una linea vertical
           
            elif((clicky-starty<=50) and startx-r<=clickx<=startx+r):
            
                TableroVertical[j][i] = "1"
                ...
                HacerCuadros(TableroHorizontal,TableroVertical,\
                                        TableroCuadros,filas,columnas,jugador,\
                                        turno)
                return canvas.create_line(startx,starty,startx,starty+50,width=3,fill=color)
                            

###############

def movida(TableroHorizontal,TableroVertical,TableroCuadros,filas,columnas,turno,jugador):

    canvas.bind("<Button-1>",MarcarLinea)

    ContinuarJugando = HacerCuadros(TableroHorizontal,TableroVertical,\
                                        TableroCuadros,filas,columnas,jugador,\
                                        turno)
...
list,filas: int,columnas: int,jugador: int,turno: bool) -> (bool):
...
# (TableroCuadros[i][j] != "1") and (TableroCuadros[i][j] != "2")))
    ...
    for i in range (N): 
        ...
        for j in range (M): 
            ...
            if ((TableroHorizontal[j][i] == "1") and \
                    (TableroVertical[j][i] == "1") and \
                    (TableroHorizontal[j+1][i] == "1") and\
                    (TableroVertical[j][i+1] == "1") and \
                    (TableroCuadros[j][i] == "0")):
                
                if (jugador == 1):
                    TableroCuadros[j][i] = "1"
                    print(TableroCuadros)
                    canvas.create_rectangle(startx,starty,startx+50,starty+50,fill='red')
                    turno = True
                    return turno
                elif (jugador == 2):
                    TableroCuadros[j][i] = "2"
                    print(TableroCuadros)
                    canvas.create_rectangle(startx,starty,startx+50,starty+50,fill='blue')
                    turno = True
                    return turno
                else:
                    quit()
