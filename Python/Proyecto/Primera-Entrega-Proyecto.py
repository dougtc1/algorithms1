...
#
# Ultima modificacion: 09/06/2014
#
...
# Definicion de procedimiento que inicializa los datos del programa
#------------------------------------------------------------------------------#
...
# all((TableroHorizontal[i][j] == "0") for i in range(filas) for j in range(columnas-1))and \
# ((TableroVertical[i][j] == "0") for i in range(filas-1) for j in range(columnas) and ((TableroCuadros[i][j] == "0") for i in range(filas-1) for j in range(columnas-1)))
 
    print ("Este procedimiento inicializa: TableroHorizontal,TableroVertical,TableroCuadros")

# Definicion de procedimiento que imprime el tablero cada vez que se modifica
#------------------------------------------------------------------------------#
...
# Postcondicion: True // Esta funcion no modifica ninguna variable que recibe, solo imprime los tableros en un formato mas amigable para el usuario
	...
	print ("Este procedimiento imprime el tablero de juego en un formato entendible para el usuario")
	

# Definicion de procedimiento que permite al jugador 1 hacer su jugada
#------------------------------------------------------------------------------#
...
# Postcondicion: len(TableroHorizontal)==filas*(columnas-1) and \
# len(TableroVertical)==(filas-1)*columnas and len(TableroCuadros)==(filas-1)*(columnas-1)"""
    ...
    print("Este procedimiento devuelve: TableroHorizontal,TableroVertical,TableroCuadros,TurnoActual")
        

# Definicion de procedimiento que permite al jugador 2 hacer su jugada
#------------------------------------------------------------------------------#
...
# Postcondicion: True // Esta funcion simplemente imprime las instrucciones del juego, \
# no modifica ninguna varriable del juego

    print ("Esta funcion imprime las instrucciones del juego")
...
# Postcondicion: TableroHorizontal[lineafila][lineacolumna] == "1"
    ...
    print ("Esta funcion devuelve: TableroHorizontal")
...
# Postcondicion: TableroVertical[lineafila][lineacolumna] == "1"
    ...
    print ("Esta funcion devuelve: TableroVertical")


# Definicion de procedimiento que marca un cuadrado en el tablero y dice de que jugador fue  
#------------------------------------------------------------------------------#
...
# and (TableroVertical[i][j+1] == "1") and (TableroCuadros[i][j] != "1") and (TableroCuadros[i][j] != "2")))
    ...
    print("Este procedimiento devuelve: TableroCuadros,turno")


# Definicion de procedimiento que permite cargar una partida especificada por el usuario
#------------------------------------------------------------------------------#
...
# Postcondicion: filas>=2 and columnas>=2
    ...
    print("Este procedimiento carga una partida y llama a la funcion inicializar con los valores leidos del archivo")
...
# Definicion de procedimiento que imprime el puntaje al momento de la partida
#------------------------------------------------------------------------------#
...
# Definicion de procedimiento que especifica el turno de juego
#------------------------------------------------------------------------------#
...
# SegundoTurno = Jugador1(TableroHorizontal,TableroVertical,TableroCuadros,filas,columnas,TurnoActual))
    ...
    PrimerTurno = Jugadorx    # Variable que hace el llamado al jugador x al que le corresponde jugar de primero
    SegundoTurno = Jugadory   # Variable que hace el llamado al jugador y al que le corresponde jugar de segundo
        	
# Nota: 'x' e 'y' vienen dados por el azar si se escoge el modo de juego de dos personas


    print ("Este procedimiento devuelve el orden de los turnos con las variables: PrimerTurno, SegundoTurno")


# Definicion de procedimiento que imprime el menu del juego y permite escoger el tipo de juego
#------------------------------------------------------------------------------#
...
# filas>=2 and columnas>=2 and len(TableroHorizontal) >=2 and len(TableroVertical) >=2 and len(TableroCuadros) >=2
   ...
   print ("Este procedimiento devuelve las siguientes variables: TableroHorizontal,TableroVertical,TableroCuadros,TipoJugador,filas,columnas")
...
# Postcondicion: (seleccion == 1) or (seleccion == 2)
    ...
    seleccion = 0    # Variable que especifica la eleccion del usuario si desea salir
...
TipoJugador = 0    # Variable entera que almacena el tipo de jugador que es jugador 2, si es humano o maquina

filas = 0    # Variable entera que especifica la cantidad de filas que se usaran en los tableros

columnas = 0    # Variable entera que especifica la cantidad de columnas que se usaran en los tableros

azar = 0    # Variable entera que especifica el jugador que empieza a jugar de primero en caso de ser humano vs humano

TurnoActual = 0    # Variable entera que especifica cual jugador esta jugando al momento

PuntajeJugador1 = 0    # Variable entera que almacena la puntuacion del jugador 1

PuntajeJugador2 = 0     # Variable entera que almacena la puntuacion del jugador 2

MovimientosMaximos = 0    # Variable entera que almacena la cantidad maxima de rayas posibles
